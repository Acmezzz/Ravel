...
const read = (rel) => readFile(new URL(rel, import.meta.url), "utf8");
 ...
 test("remaining UI audit contracts cover recovery, focus, grouping, and disclosure", async () => {
  ...
  assert.match(workbench, /aria-controls="omega-right-drawer"/);
  assert.match(workbench, /omega-left-rail/);
  assert.match(workbench, /展开左侧导航/);
...
test("fork, tree overlay, and model picker exist as surfaces", async () => {
  ...
  assert.match(picker, /前往模型中心/);
  assert.match(picker, /setModelCenterOpen\(true\)/);
...
test("Project Switcher, replay, and worker recovery reconcile surfaces exist", async () => {
  ...
  const header = await read("../src/renderer/components/layout/Header.tsx");
  const titleBar = await read("../src/renderer/components/layout/TitleBar.tsx");
  const trustCenter = await read("../src/renderer/components/layout/TrustCenter.tsx");
  assert.match(header, /retryWorker/);
  assert.match(titleBar, /RAVEL DESKTOP/);
  assert.match(titleBar, /∞/);
  assert.match(trustCenter, /pendingChoice/);
  assert.match(trustCenter, /workspace\.trust === "trusted"/);
