...
const releaseDir = resolve(process.env.RAVEL_RELEASE_DIR ?? process.env.OMEGA_RELEASE_DIR ?? join(process.cwd(), "release", "win-unpacked"));
const executable = process.platform === "win32" ? join(releaseDir, "Ravel Desktop.exe") : join(releaseDir, "Ravel Desktop");
const runtimeRoot = join(releaseDir, "resources", "ravel-runtime");
const timeoutMs = Number(process.env.RAVEL_SMOKE_TIMEOUT_MS ?? process.env.OMEGA_SMOKE_TIMEOUT_MS ?? 45_000);
