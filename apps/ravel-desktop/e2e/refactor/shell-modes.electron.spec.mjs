 ...
 *  5. Focus Mode：Header「进入专注模式」切换 `.ravel-workbench[data-focus-mode]`。
...
test("Shell 三模式切换与 active session 保持", async () => {
  ...
  try {
    ...
    await expect(page.locator('[data-surface="chat-chat"]')).toHaveCount(0);
...
test("Shell 活动栏、Focus Mode 与缩放", async () => {
  ...
  try {
    ...
    const rail = page.locator(".ravel-shell-rail");
    ...
    await expect(page.locator('.ravel-workbench[data-focus-mode="true"]')).toBeVisible();
    await page.getByRole("button", { name: /退出专注模式/ }).click();
    await expect(page.locator('.ravel-workbench[data-focus-mode="false"]')).toBeVisible();
