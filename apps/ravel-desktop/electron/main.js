...
const RAVEL_DATA_DIR = "ravel";
const RAVEL_RUNTIME_DIR = "ravel-runtime";

function firstEnv(...names) {
  return names.map((name) => process.env[name]).find((value) => typeof value === "string" && value.length > 0) ?? null;
}

let startupRequest = { workspace: firstEnv("RAVEL_WORKSPACE", "OMEGA_WORKSPACE"), sessionId: null };
...
function parseStartupRequest(argv = process.argv) {
  const result = { workspace: firstEnv("RAVEL_WORKSPACE", "OMEGA_WORKSPACE"), sessionId: null };
...
function rootOf() {
  return app.isPackaged ? (process.resourcesPath ? join(process.resourcesPath, RAVEL_RUNTIME_DIR) : app.getAppPath()) : DEV_ROOT;
}

function extensionsRootOf() {
  return firstEnv("RAVEL_EXTENSIONS_ROOT", "OMEGA_EXTENSIONS_ROOT") ?? (app.isPackaged ? join(rootOf(), ".pi", "extensions") : join(DEV_ROOT, ".pi", "extensions"));
}

function dataRoot() {
  return join(app.getPath("userData"), RAVEL_DATA_DIR);
}
...
function desktopSettingsFile() {
  return join(dataRoot(), "desktop-settings.json");
}

function credentialStoreFile() {
  return join(dataRoot(), "credentials.bin.json");
}

function recentEventsFile(sessionId) {
  return join(dataRoot(), "event-cache", `${String(sessionId).replace(/[^A-Za-z0-9_-]/g, "_")}.jsonl`);
}
...
function bindHost(host) {
  ...
  host.onSettled = () => {
    ...
    try {
      new Notification({ title: "Ravel Desktop", body: "回复已完成，点击查看" }).show();
    } catch {
      /* best effort */
    }
  };
...
async function createWindow() {
  ...
  win.webContents.on("render-process-gone", (_event, details) => {
    ...
    void dialog.showMessageBox(win, { type: "error", title: "Ravel 渲染器已停止", message: "界面进程已崩溃，可以重新加载界面。", detail: `原因：${reason}`, buttons: ["重新加载", "退出"] }).then(({ response }) => {
      if (!win || win.isDestroyed()) return;
  ...
  if ((process.env.RAVEL_AUTOTEST ?? process.env.OMEGA_AUTOTEST) === "1" && worker) {
    ...
    setTimeout(async () => {
      if ((process.env.RAVEL_DOMPROBE ?? process.env.OMEGA_DOMPROBE) && win && !win.isDestroyed()) {
        try {
      ...
      const forcedTheme = firstEnv("RAVEL_THEME", "OMEGA_THEME");
      if (forcedTheme && win && !win.isDestroyed()) {
        try {
          await win.webContents.executeJavaScript(
            `localStorage.setItem("ravel-theme", ${JSON.stringify(JSON.stringify(forcedTheme))})`,
          );
      ...
      const screenshotPath = firstEnv("RAVEL_SCREENSHOT", "OMEGA_SCREENSHOT");
