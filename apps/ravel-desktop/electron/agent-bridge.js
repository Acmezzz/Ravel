...
function extensionsRootOf(value) {
  const root = value ?? process.env.RAVEL_EXTENSIONS_ROOT ?? process.env.OMEGA_EXTENSIONS_ROOT ?? DEV_EXTENSIONS_ROOT;
  if (!existsSync(root)) throw new Error(`Ravel extensions directory not found: ${root}`);
  return root;
}
