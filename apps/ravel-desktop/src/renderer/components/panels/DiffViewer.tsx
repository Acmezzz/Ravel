...
function FileCard({ file, selection, onToggleFile, onToggleHunk, onOpenFile }: { file: DiffFile; selection: Selection; onToggleFile: (path: string) => void; onToggleHunk: (path: string, hunkIndex: number) => void; onOpenFile: (path: string) => void }): React.ReactElement {
  ...
  return (
    <article className={`omega-diff-file${expanded ? " is-expanded" : ""}`}>
      ...
      {expanded ? <div className="omega-diff-file-body">{file.hunks.slice(0, MAX_RENDERED_HUNKS_PER_FILE).map((hunk, index) => <section key={index} className="omega-diff-hunk"><button type="button" className="omega-diff-hunk-header" onClick={() => onToggleHunk(file.path, index)}><input className="omega-checkbox" type="checkbox" checked={wholeFile || (selectedHunks?.has(index) ?? false)} aria-label={t("diff.hunkAria", { path: file.path, index: index + 1 })} onClick={(event) => event.stopPropagation()} onChange={() => onToggleHunk(file.path, index)} /><span>{hunk.header}</span></button><div className="omega-diff-lines">{hunk.lines.slice(0, MAX_RENDERED_LINES_PER_HUNK).map((line, lineIndex) => <HunkLine key={lineIndex} line={line} />)}{hunk.lines.length > MAX_RENDERED_LINES_PER_HUNK ? <p className="omega-diff-truncated">{t("diff.foldedLines", { n: hunk.lines.length - MAX_RENDERED_LINES_PER_HUNK })}</p> : null}</div></section>)}</div> : null}
    </article>
  );
}
