...
export function ResourceCenter(): React.ReactElement {
	...
	return <Dialog open={open} onOpenChange={setOpen}><DialogContent className="omega-dialog-wide"><DialogTitle>资源中心</DialogTitle><div className="omega-dialog-content-area omega-resource-content"><p className="omega-muted-text">管理当前会话加载的扩展、Skills 和 Prompt 模板。安装只接受本机路径，不会联网下载 npm/git 包。</p>{bundle?.projectTrusted === false ? <p className="omega-warning-text">当前项目未信任，项目级资源处于休眠状态。可在项目切换器中重新设置信任。</p> : null}<div className="omega-button-row"><Button size="sm" variant="solid" disabled={busy} onClick={() => void apply(() => ipc.installLocalResource({ project: false }), "已安装到用户范围")}>安装到用户</Button><Button size="sm" disabled={busy || bundle?.projectTrusted === false} onClick={() => void apply(() => ipc.installLocalResource({ project: true }), "已安装到项目范围")}>安装到项目</Button><Button size="sm" disabled={busy} onClick={() => { setRemoteStaged(null); setRemoteUrl(""); setRemoteFormOpen(true); }}>从 URL 安装</Button><Button size="sm" disabled={busy} onClick={() => void apply(() => ipc.reloadResources(), "已重载当前会话资源")}>重载资源</Button>{busy ? <span className="omega-spinner" role="status" aria-label="处理中" /> : null}</div><div className="omega-setting-row"><div><strong>允许 /skill 调用</strong><span>模型可通过 slash 命令触发 Skills</span></div><Switch checked={bundle?.skillCommandsEnabled !== false} disabled={busy || !bundle} onCheckedChange={(enabled) => void apply(() => ipc.setSkillCommandsEnabled({ enabled }), "已更新 skill 命令")} /></div><TextField aria-label="搜索扩展、skill、prompt" placeholder="搜索扩展、skill、prompt…" value={query} onChange={(event) => setQuery(event.target.value)} />{error ? <p className="omega-error-text">{error}</p> : null}{status ? <p className="omega-status-text">{status}</p> : null}
		...
		<div className="omega-section-heading"><h3 className="overline-label">MCP 服务器（stdio）</h3><Button size="sm" disabled={busy} onClick={() => { setMcpName(""); setMcpCommand(""); setMcpArgs(""); setMcpScopeProject(false); setMcpFormOpen(true); }}>添加</Button></div>{mcpBundle && !mcpBundle.bridgeLoaded ? <p className="omega-warning-text">执行桥（ravel-mcp-bridge 扩展）未加载：定义可以保存，但工具不会出现在会话中，启用状态不会生效。</p> : null}{mcpServers.length === 0 ? <EmptyText>还没有本地 stdio MCP 定义。</EmptyText> : mcpServers.map((server) => <ResourceRow key={`${server.scope}:${server.name}`} title={server.name} subtitle={[server.command, ...server.args].join(" ")} chips={[scopeLabel(server.scope)]} enabled={server.enabled} busy={busy} onToggle={(enabled) => void applyMcp(() => ipc.mcpSetEnabled({ name: server.name, enabled, project: server.scope === "project" }), enabled ? "已启用 MCP 服务器" : "已禁用 MCP 服务器")} extra={<Button size="sm" disabled={busy} onClick={() => setRemovingMcp(server)}>移除</Button>} />)}</div><DialogFooter><Button variant="solid" onClick={() => setOpen(false)}>完成</Button></DialogFooter></DialogContent>
		...
		<Dialog open={mcpFormOpen} onOpenChange={setMcpFormOpen}><DialogContent><DialogTitle>添加 MCP 服务器</DialogTitle><div className="omega-dialog-content-area omega-form-stack"><p className="omega-muted-text">仅支持本机 stdio 服务器。只添加你信任的服务器——它的工具将以 untrusted 权限档进入审批流程。</p><TextField label="名称" value={mcpName} onChange={(event) => setMcpName(event.target.value)} placeholder="github-context" /><TextField label="可执行命令" value={mcpCommand} onChange={(event) => setMcpCommand(event.target.value)} placeholder="npx -y @modelcontextprotocol/server-git" /><TextField label="参数（每行一个）" value={mcpArgs} onChange={(event) => setMcpArgs(event.target.value)} multiline minRows={2} /><TextField label="范围" select value={mcpScopeProject ? "project" : "user"} onChange={(event) => setMcpScopeProject(event.target.value === "project")}><option value="user">用户（~/.ravel/mcp.json）</option><option value="project">项目（&lt;workspace&gt;/.ravel/mcp.json，需已信任）</option></TextField></div><DialogFooter><Button onClick={() => setMcpFormOpen(false)}>取消</Button><Button variant="solid" disabled={busy || !mcpName.trim() || !mcpCommand.trim()} onClick={() => void applyMcp(() => ipc.mcpAdd({ name: mcpName, command: mcpCommand, args: mcpArgs.split("\n").map((line) => line.trim()).filter(Boolean), project: mcpScopeProject }), "已保存 MCP 定义").then(() => setMcpFormOpen(false))}>保存</Button></DialogFooter></DialogContent></Dialog>
