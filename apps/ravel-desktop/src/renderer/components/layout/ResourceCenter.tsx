...
export function ResourceCenter(): React.ReactElement {
	...
	const [installMenuOpen, setInstallMenuOpen] = React.useState(false);
	...
	// Four resource sections used to each print their own "没有匹配的 XX。" line,
	// so an empty install showed the same sentence four times and ate a third of
	// the dialog. Collapse them into one empty state instead.
	const allResourcesEmpty = extensions.length === 0 && skills.length === 0 && prompts.length === 0 && packages.length === 0;
	const emptyHint = query.trim()
		? `没有匹配「${query.trim()}」的资源。`
		: "还没有安装任何资源。用上方「安装」从本机路径添加，或选择从 URL / Registry 安装。";
	return <Dialog open={open} onOpenChange={setOpen}><DialogContent className="omega-dialog-wide"><DialogTitle>资源中心</DialogTitle><div className="omega-dialog-content-area omega-resource-content"><p className="omega-muted-text">管理当前会话加载的扩展、Skills 和 Prompt 模板。安装只接受本机路径，不会联网下载 npm/git 包。</p>{bundle?.projectTrusted === false ? <p className="omega-warning-text">当前项目未信任，项目级资源处于休眠状态。可在项目切换器中重新设置信任。</p> : null}<div className="omega-button-row"><Menu open={installMenuOpen} onOpenChange={setInstallMenuOpen}><MenuAnchor asChild><Button size="sm" variant="solid" disabled={busy} trailing={<ChevronDown size={14} strokeWidth={1.9} aria-hidden="true" />} aria-haspopup="menu" aria-expanded={installMenuOpen} onClick={() => setInstallMenuOpen(true)}>安装</Button></MenuAnchor><MenuContent style={{ minWidth: 200 }}><MenuItem onSelect={() => void apply(() => ipc.installLocalResource({ project: false }), "已安装到用户范围")}>安装到用户</MenuItem><MenuItem disabled={bundle?.projectTrusted === false} onSelect={() => void apply(() => ipc.installLocalResource({ project: true }), "已安装到项目范围")}>安装到项目</MenuItem></MenuContent></Menu><Button size="sm" disabled={busy} onClick={() => { setRemoteStaged(null); setRemoteUrl(""); setRemoteFormOpen(true); }}>从 URL 安装</Button><Button size="sm" disabled={busy} onClick={() => { setRegistryEntries(null); setRegistryStaged([]); setRegistryFormOpen(true); }}>从 Registry 安装</Button><Button size="sm" disabled={busy} onClick={() => void apply(() => ipc.reloadResources(), "已重载当前会话资源")}>重载资源</Button>{busy ? <span className="omega-spinner" role="status" aria-label="处理中" /> : null}</div><div className="omega-setting-row"><div><strong>允许 /skill 调用</strong><span>模型可通过 slash 命令触发 Skills</span></div><Switch checked={bundle?.skillCommandsEnabled !== false} disabled={busy || !bundle} onCheckedChange={(enabled) => void apply(() => ipc.setSkillCommandsEnabled({ enabled }), "已更新 skill 命令")} /></div><TextField aria-label="搜索扩展、skill、prompt" placeholder="搜索扩展、skill、prompt…" value={query} onChange={(event) => setQuery(event.target.value)} />{error ? <p className="omega-error-text">{error}</p> : null}{status ? <p className="omega-status-text">{status}</p> : null}
		{allResourcesEmpty ? <EmptyText>{emptyHint}</EmptyText> : <React.Fragment><h3 className="overline-label">扩展（{extensions.length}）</h3>{extensions.length === 0 ? <EmptyText>没有匹配的扩展。</EmptyText> : extensions.map((extension: ExtensionResource) => <ResourceRow key={extension.path} title={extension.name} subtitle={extension.path} chips={[scopeLabel(extension.scope), extension.origin === "package" ? "包" : "本地", extension.commands ? `${extension.commands} 命令` : "", extension.tools ? `${extension.tools} 工具` : ""].filter(Boolean)} enabled={extension.enabled !== false} dormant={extension.dormant} busy={busy} onToggle={(enabled) => void apply(() => ipc.setResourceEnabled({ kind: "extension", path: extension.path, enabled, project: extension.scope === "project", baseDir: extension.baseDir }), enabled ? "已启用扩展" : "已禁用扩展")} />)}
