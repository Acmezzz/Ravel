...
export function ExtensionSurface(): React.ReactElement | null {
  const { sessionId, statuses, widgets } = useAppStore((state) => ({ sessionId: state.activeSessionId, statuses: state.extensionStatuses.filter((item) => item.sessionId === state.activeSessionId), widgets: state.extensionWidgets.filter((item) => item.sessionId === state.activeSessionId) }), shallow);
