...
> 当前验证：Electron syntax、Renderer TypeScript、Vite build、offline SDK smoke、桌面安全测试 **123/123**、release gate 均通过。
>
...
- Model Center 已支持本地 custom provider 配置持久化与 Worker 重启恢复：id/name/baseUrl/API 类型/headers/model 定义/上下文窗口，并通过 Worker `registerProvider` 组合到离线 runtime；真实 OAuth、在线 discovery、真实 latency 仍为外部依赖。
