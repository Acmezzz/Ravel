...
import { addWorktree, listWorktrees, parseWorktreeList } from "../electron/diff-service.js";
...
test("addWorktree refuses paths outside the repository parent", async () => {
  const root = await mkdtemp(join(tmpdir(), "omega-worktree-root-"));
  execFileSync("git", ["init", "-q"], { cwd: root });
  await writeFile(join(root, "README.md"), "hello");
  execFileSync("git", ["add", "README.md"], { cwd: root });
  execFileSync("git", ["-c", "user.email=omega@example.com", "-c", "user.name=Omega", "commit", "-qm", "initial"], { cwd: root });
  const outside = await mkdtemp(join(tmpdir(), "omega-worktree-outside-"));
  assert.throws(
    () => addWorktree(root, { path: join(outside, "stolen"), branch: "feat", createBranch: true }),
    (error) => error.code === "workspace_not_authorized",
  );
});
